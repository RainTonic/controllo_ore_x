...
export class AuthService {
  ...
  constructor(
    ...
    this._jsonwebtokenSecret =
      this._configService.get<string>('JWT_SIGN_SECRET');
    this._tokenExpirationMinutes =
      this._configService.get<number>('TOKEN_EXPIRATION_MINUTES') ||
      AuthService._DEFAULT_TOKEN_EXPIRATION_MINUTES;
    this._isTokenExpirationEnabled =
      this._configService.get<string>('ENABLE_TOKEN_EXPIRATION') === 'true';
  }
  ...
  async authenticate(token: string): Promise<User> {
    const loginToken: AuthToken = await this._authTokenRepository.findOne({
      where: { token },
    });
    if (!loginToken) {
    ...
    if (
      this._isTokenExpirationEnabled &&
      isBefore(loginToken.validUntil, new Date())
    ) {
  ...
  async login(body: LoginDtoV): Promise<{ user: User; token: string }> {
    const currentUser: User = await this._userService.getUserByEmailAndPassword(
      body.email,
      body.password,
    );
    return {
  ...
  async getMeByToken(token: string): Promise<User> {
    const tokenEntity: AuthToken = await this._authTokenRepository.findOne({
      where: { token },
    ...
    if (
      this._isTokenExpirationEnabled &&
      isBefore(tokenEntity.validUntil, new Date())
    ) {
      throw new GoneException(ApiErrors.EXPIRED_TOKEN);
    }
